describe('PlayersService', function () {
  ...
  describe('getPlayerList', function () { //Get playerList es el metodo a probar
  ...
  describe('substractPlayer', function () {
    it('should substract 5 points to the player score with the given id', function () {
      var playerId = 1;
      spyOn(Players, 'update');

      PlayersService.substractPlayer(playerId);
      expect(Players.update.calls.argsFor(0)).toEqual([playerId, {$inc: {score: -5}}]);
    });
  });
  ...
  describe('deletePlayer', function () {
    it('should remove a player with the given id', function () {
      ...
      expect(PlayersService.getPlayer(playerId)).toBe(undefined);
    });
  });

...
PlayersService = {
  ...
  substractPlayer: function (playerId) {
    Players.update(playerId, {$inc: {score: -5}});
  },
...
if (Meteor.isClient) {
  ...
  Template.leaderboard.events({
    ...
    'click input.dec': function () {
      PlayersService.substractPlayer(Session.get("selected_player"));
    },
    'click input.del': function () {
      var playerId = (PlayersService.getPlayer(Session.get("selected_player")))._id;
      PlayersService.deletePlayer(playerId);
    },
  	'submit form': function (event) {
  		event.preventDefault();
  		var newName = $('#newPlayer').val();
  		console.log(newName);
  		Players.insert({
  			name: newName,
  			score: 0
